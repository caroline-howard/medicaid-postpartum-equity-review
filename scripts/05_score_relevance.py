...
TERM_GROUPS = {
    "medicaid_chip": ["medicaid", "chip", "children health insurance"],
    "postpartum": ["postpartum", "post partum", "postnatal", "pregnancy", "maternal"],
    "coverage_policy": ["coverage", "eligibility", "extension", "continuous coverage", "churn", "redetermination", "waiver", "state plan"],
    "outcomes": ["access", "continuity", "behavioral health", "morbidity", "mortality", "equity", "disparities", "postpartum visit", "emergency department"],
    "us_state_policy": ["united states", "u s", "state", "medicaid agency", "section 1115", "cms"],
}
...
def main() -> None:
    ...
    for _, row in records.iterrows():
        score, details = score_text(f"{row.get('title', '')} {row.get('abstract', '')}")
        updated = row.to_dict()
