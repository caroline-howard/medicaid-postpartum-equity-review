...
The pipeline retrieves candidate records from PubMed and OpenAlex, enriches metadata with Crossref, combines and deduplicates records, applies transparent relevance scoring, prepares human screening files, tracks policy/context sources, and builds PRISMA-style count outputs. It does not make final scholarly inclusion decisions.
...
python scripts/02_openalex_search.py
...
- `data/raw/openalex_records.csv`: candidate records retrieved from OpenAlex.
- `data/processed/deduplicated_records.csv`: combined candidate records after DOI, PMID, and normalized-title deduplication.
...
There is no required number of final included studies. Final inclusion depends on eligibility criteria and documented human screening decisions. Practical record-volume targets are used only as quality checks: approximately 100-400 candidate records before deduplication is a useful initial target; fewer than 50 PubMed and OpenAlex records combined should be flagged as potentially too narrow; more than 750 should be flagged as potentially too broad; and a feasible portfolio systematic review often has about 10-25 core evidence sources plus separately labeled policy/context sources.
